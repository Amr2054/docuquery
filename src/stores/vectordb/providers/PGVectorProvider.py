...
class PGVectorProvider(VectorDBInterface):
    ...
    # Every Collection (project) is saved in one Table
    async def is_collection_existed(self, collection_name: str) -> bool:
        ...
        async with self.db_client() as session:
            async with session.begin():
                ...
                results = await session.execute(list_tbl, {"collection_name": collection_name}) # Safe SQL Query
    ...
    async def get_collection_info(self, collection_name: str) -> dict:
        async with self.db_client() as session:
            async with session.begin():
                ...
                table_data = table_info.fetchone() # return one record
                if not table_data:
                    return None
    ...
    async def delete_collection(self, collection_name: str):
        async with self.db_client() as session:
            async with session.begin():
                ...
                await session.commit() # Commit only when changing the database
    ...
    async def is_index_existed(self, collection_name: str) -> bool:
        ...
        async with self.db_client() as session:
            async with session.begin(): # pg_indexes is a pg special table has all indexes
    ...
    async def create_vector_index(self, collection_name: str,
        ...
        async with self.db_client() as session:
            async with session.begin():
                ...
                ) # We use column name (vector) and distance method to create the indexes

                await session.execute(create_idx_sql)

                self.logger.info(f"END: Created vector index for collection: {collection_name}")

    # Used to refresh indexes after alot of new data is inserted which may be classified to wrong clusters due to strict clusters built on older variations in data
        ...
        async with self.db_client() as session:
            async with session.begin():
                ...
                await session.execute(insert_sql, {
                    'text': text,
                    'vector': "[" + ",".join([str(v) for v in vector]) + "]", # postgres : have to convert vector from list to string first
        ...
        async with self.db_client() as session:
            async with session.begin():
                for i in range(0, len(texts), batch_size):
                    ...
                    for _text, _vector, _metadata, _record_id in zip(batch_texts, batch_vectors, batch_metadata,
                                                                     batch_record_ids):
                    ...
                    batch_insert_sql = sql_text(f'INSERT INTO {collection_name} '
                                                ...
                                                f'VALUES (:text, :vector, :metadata, :chunk_id)') # No Bracket means insert batch

                    await session.execute(batch_insert_sql, values)
        # Call after finishing the main session in insert_many
        await self.create_vector_index(collection_name=collection_name)
    ...
    async def search_by_vector(self, collection_name: str, vector: list, limit: int):
        ...
        async with self.db_client() as session:
            async with session.begin():
                search_sql = sql_text(
                    f'SELECT {PgVectorTableSchemeEnums.TEXT.value} as text, 1 - ({PgVectorTableSchemeEnums.VECTOR.value} <=> :vector) as score' # <=> means compare 
